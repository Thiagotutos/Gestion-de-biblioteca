...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_item'])) {
    ...
    if ($name) {
        $stmt = $pdo->prepare("INSERT INTO racks (name) VALUES (?)");
        try { $stmt->execute([$name]); $_SESSION['success_msg'] = "Estante agregado."; } 
        catch (Exception $e) { $error = "El estante ya existe."; }
        header('Location: racks.php'); exit;
    }
}

if (isset($_GET['delete'])) {
    if ($_SESSION['user_role'] === 'Lector') { header('Location: racks.php'); exit; }
    $id = $_GET['delete'];
    $pdo->prepare("UPDATE books SET rack_id = NULL WHERE rack_id = ?")->execute([$id]);
    $pdo->prepare("DELETE FROM racks WHERE id = ?")->execute([$id]);
    $_SESSION['success_msg'] = "Estante eliminado.";
    header('Location: racks.php'); exit;
}
...
$count_stmt = $pdo->prepare("SELECT COUNT(*) FROM racks $where");
$count_stmt->execute($params);
$total = $count_stmt->fetchColumn();
$total_pages = max(1, ceil($total / $per_page));

$stmt = $pdo->prepare("SELECT * FROM racks $where ORDER BY name ASC LIMIT $per_page OFFSET $offset");
$stmt->execute($params);
$racks = $stmt->fetchAll();
$url_params = [];
...
<div class="content-header">
    <h2>Gestionar Estantes</h2>
    <span id="result-count" style="color:#666;font-size:0.9rem;"><?php echo number_format($total); ?> estantes</span>
</div>
...
<div style="display:flex;gap:20px;">
    ...
    <div class="form-container" style="flex:1;height:max-content;">
        <h3>Agregar Estante</h3><br>
        <form method="POST">
            <div class="form-group"><label>Nombre del Estante</label>
    ...
    <div style="flex:2;">
        <form method="GET" class="search-filters-bar" style="margin-bottom:15px;">
            <div class="search-input-wrapper">
                ...
                <input type="text" name="search" id="liveSearchInput" class="search-input" placeholder="Buscar estante..." value="<?php echo htmlspecialchars($search); ?>" autocomplete="off">
                <span id="searchSpinner" class="search-spinner" style="display:none;"></span>
            </div>
            <button type="submit" class="btn btn-primary">Buscar</button>
            <?php if ($search !== ''): ?><a href="racks.php" class="btn" style="background:#999;">Limpiar</a><?php endif; ?>
        </form>
        ...
        <table class="data-table"><thead><tr><th>ID</th><th>Nombre</th><th>Libros</th><th>Acción</th></tr></thead>
            <tbody>
                <?php foreach ($racks as $item): 
                    $bc = $pdo->prepare("SELECT COUNT(*) FROM books WHERE rack_id = ?");
                ...
                <tr>
                    ...
                    <?php if ($_SESSION['user_role'] !== 'Lector'): ?>
                    <td><a href="racks.php?delete=<?php echo $item['id']; ?>" class="btn btn-delete" onclick="return confirm('¿Borrar estante?');">Borrar</a></td>
                    <?php else: ?><td>-</td><?php endif; ?>
                </tr>
                <?php endforeach; ?>
                <?php if (empty($racks)): ?><tr><td colspan="4" style="text-align:center;">No hay estantes.</td></tr><?php endif; ?>
            </tbody>
        </table>
...
(function(){
    ...
    function search(){
        ...
        xhr.open('GET','search_ajax.php?type=racks&q='+encodeURIComponent(si.value)+'&page=1',true);
        xhr.send();
    }
    function bindPag(){
    ...
    function loadP(pg){
        ...
        xhr.open('GET','search_ajax.php?type=racks&q='+encodeURIComponent(si.value)+'&page='+pg,true);
        xhr.send();
    }
